...
- Install : `bun install`
- Dev server : `bun dev` ([http://localhost:3000](http://localhost:3000))
...
passage au moteur (`manche/lecture.ts`), et les fonctions pures des écrans —
ce que chaque mode **demande** et comment ça se dit (`gestes.ts` : les deux
espèces de gestes, les bornes du pavé, les mots d'une case), par où la passe
avant démarre (`passe-avant.ts`), ce que le pavé accepte touche à touche
(`pave.ts`). `gestes.ts` ne touche pas la base : c'est ce qui laisse les
composants client l'importer pour mettre une valeur en mots. Le **tiroir** est la lecture du journal : sa part pure — adresse,
